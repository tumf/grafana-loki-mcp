...
def test_iso8601_to_unix_nano():
    ...
    dt_z = datetime.datetime.fromisoformat(iso.replace("Z", "+00:00"))
...
def test_query_loki(mock_get, grafana_client, mock_response):
    ...
    with patch.object(GrafanaClient, "get_datasources", return_value={"datasources": [{"uid": "test-uid", "type": "loki"}]}):
        # Test with UNIX ns
        result = grafana_client.query_loki("{job='test'}", start="1716800000000000000", end="1716803600000000000")
        assert "data" in result
        # Test with ISO8601
        iso_start = "2025-05-27T14:59:33.073316"
        iso_end = "2025-05-27T15:00:00.000000"
        result2 = grafana_client.query_loki("{job='test'}", start=iso_start, end=iso_end)
        assert "data" in result2
        # Ensure conversion was attempted (params should be int for ISO8601)
        call_args = mock_get.call_args_list[-1][1]["params"]
        assert isinstance(call_args["start"], int)
        assert isinstance(call_args["end"], int)
